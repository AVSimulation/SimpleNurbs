...
/*!
  \brief Find the minimal distance between a point and the surface.
	...
	Adapted from minDist2 to support closed NURBS
	...
	*/
	template <class T, int N>
	T ParaSurface<T,N>::minDist2Loop(const Point_nD<T,N>& p, T& guessU, T& guessV,T error,T s,int sep,int maxIter) const {
		T d,d1,d2 ;
		Point_nD<T,N> p2 ;
		p2 = pointAt(guessU,guessV) ;
		d = norm2(p-p2) ;
		...
		while(d>error && niter<maxIter) {
			...
			for(u=u1;u<u2;u+=step)
				for(v=v1;v<v2;v+=step)
				{
					T umod = u;
					T vmod = v;
					while(umod<0.0) umod+=1.0;
					while(umod>1.0)	umod-=1.0;
					while(vmod<0.0)	vmod+=1.0;
					while(vmod>1.0)	vmod-=1.0;

					p2 = pointAt(umod,vmod) ;
					d1 = norm2(p-p2) ;
					if(d1<d)
					{
						d = d1 ;
