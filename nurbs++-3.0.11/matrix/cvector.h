...
namespace PLib {
  ...
  */
  ...
  public:
    ...
    T& operator[](const int i) { return Vector<T>::x[i%Vector<T>::sze]; }
    T  operator[](const int i) const   { return Vector<T>::x[i%Vector<T>::sze]; }
    
    void put(T v) { Vector<T>::x[index] = v ; index = (index+1)%Vector<T>::sze; }
